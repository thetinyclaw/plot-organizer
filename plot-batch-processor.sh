...
for zip_file in "$RAW_DATA_DIR"/*.zip; do
    ...
    signal_summary=""
    signal_output=$(python3 "$SIGNAL_INJECTION_SCRIPT" "$config_file" "$MFG_CRITERIA" --channel-map "$CHANNEL_MAP" --output-dir "$analysis_dir" 2>&1) && signal_success=true || { log "WARNING: Signal injection failed for $part_id (known bug - continuing)"; signal_success=false; }
    signal_summary=$(echo "$signal_output" | tail -n 1)
    log " Signal summary: $signal_summary"
    ...
    log " [4/4] Generating PDF report..."
    cd "$PLOT_ORGANIZER_DIR"
    python ./scripts/process_data.py --dir "$analysis_dir" --output "$analysis_dir" --summary "$signal_summary" || { log "ERROR: PDF generation failed for $part_id (some analysis steps may have failed)"; }
